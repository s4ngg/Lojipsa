...
## 11) 프리 티어 EC2(t3.micro, 1GB RAM)에서 Docker 빌드 중 인스턴스 자체가 응답 불능

**문제 상황**

배포 마지막 단계로 EC2에서 `docker compose -f deploy/docker-compose.prod.yml up -d --build`를
실행했다. 빌드 로그는 `:compileJava`(51.75초) → `:processResources` → `:classes` →
`:resolveMainClassName`(61.43초)까지는 정상적으로 진행됐는데, 그 이후로 출력이 완전히
멈췄다. 백그라운드로 넘겨 SSH 재접속을 6번(10초 타임아웃) + 1번(60초 타임아웃) 시도했지만
전부 `Connection timed out during banner exchange`로 실패했다. 그런데 AWS 콘솔에서는
인스턴스 상태가 계속 "실행 중"이고 상태 검사도 "3/3 검사 통과"로 떠 있어서, 겉보기엔
아무 이상이 없는 것처럼 보였다.

**원인 분석**

인스턴스 상태 검사(status check)는 하이퍼바이저·네트워크 레벨의 헬스체크일 뿐, OS 내부에서
sshd가 실제로 요청에 응답할 수 있는지는 보장하지 않는다는 게 핵심이었다. `docker compose
--build`가 EC2 안에서 `eclipse-temurin:21-jdk` 이미지로 Gradle(`./gradlew bootJar`)을 직접
실행하는 구조였는데, t3.micro는 메모리가 1GB뿐이고 스왑도 설정돼 있지 않았다. JDK 컴파일러 +
Gradle 데몬 + 이미 떠 있던 백엔드 컨테이너가 동시에 메모리를 요구하면서 OOM 상황에 가깝게
몰렸고, 커널이 메모리 회수에 매달리는 동안 sshd를 포함한 다른 프로세스들도 새 연결에 응답할
여유가 없어진 것으로 판단했다. 빌드를 로컬(개발 PC)에서 안 하고 굳이 1GB짜리 서버에서 직접
돌리는 구조 자체가 근본 원인이었다.

**해결 방법**

빌드만 끝낼 목적으로 인스턴스를 일시적으로 t3.small(2GB)로 업사이즈했다. 다만 인스턴스가
이미 응답 불능 상태라 일반적인 "인스턴스 중지"(OS에 정상 종료 신호를 보내고 응답을 기다림)
자체가 먹통인 OS 때문에 걸릴 수 있어서, "OS 종료 건너뛰기" 옵션으로 강제 중지한 뒤 인스턴스
유형을 t3.small로 변경하고 재시작했다. 이번엔 빌드가 걸림 없이 46.9초 만에 `BUILD SUCCESSFUL`로
끝났고 컨테이너도 정상 기동됐다. 이미지가 한 번 빌드돼 캐시된 뒤에는, `docker-compose.prod.yml`에
`restart: unless-stopped`가 걸려 있어서 인스턴스를 다시 t3.micro로 낮추고 재시작해도
재빌드 없이 컨테이너가 자동으로 다시 떴다 — 상시 운영은 비용이 싼 t3.micro로, 빌드처럼
순간적으로 메모리를 많이 쓰는 작업만 잠깐 사양을 올렸다 내리는 방식으로 정리했다.

**결과**

t3.small에서 빌드 성공 후 `curl localhost:8080/api/public/status`로 정상 응답을 확인했고,
t3.micro로 되돌린 뒤 재시작해서도(재빌드 없이) 같은 헬스체크가 정상 응답하는 것을 확인했다.
이후 인스턴스가 재시작되면 퍼블릭 IP가 바뀌는 것도 함께 확인해서, Route 53의 `api` A 레코드와
Discord OAuth redirect URI를 새 IP/도메인 기준으로 다시 맞춰줘야 했다. "상태 검사 통과 = 서버가
응답한다"가 아니라는 걸, 그리고 리소스 제약이 있는 프리 티어에서 무거운 빌드를 직접 돌리는 건
피하거나(로컬/CI 빌드 후 이미지만 배포) 최소한 일시적 스케일업 전략이 필요하다는 걸 확인한
사례였다.
