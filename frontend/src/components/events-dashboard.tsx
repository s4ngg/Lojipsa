...
import { AdminAuthError, fetchEventStatus, type EventStatus } from '@/lib/api'
...
export function EventsDashboard({
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const data = await fetchEventStatus(token)
        if (!cancelled) {
          setStatus(data)
          setError(null)
        }
    ...
    <>
      <header className="flex h-11 shrink-0 items-center justify-between border-b border-border px-4">
        <div className="flex items-center gap-2 text-[12px]">
          ...
          <span className="font-medium">일정 알리미</span>
          {status && !error && (
      ...
      <div className="flex-1 overflow-y-auto p-4">
        ...
        {status && (
          <EventsPanel
            watchedCategories={status.watchedCategories}
            reminderMinutesBefore={status.reminderMinutesBefore}
